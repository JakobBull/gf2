...
class Parser:
    ...
    def parse_network(self):
        ...
        while True:
            ...
            if self.symbol.id>=scanner.CONNECTIONS_ID and self.symbol.id<=scanner.MONITOR_ID #Check if symbol is a Heading
    ...
    def monitor_parse(self):
        ...
        if self.symbol.type != self.scanner.SEMICOLON:
            self.parse_errors += 1
            raise SyntaxError("Expected ; to end line")
    def phrase_detection(self, phrase_type_list):
        """
        ...
        #length of phrase
        len = len(phrase_type_list)
        for i in range(len):
            # Call for the next symbol from scanner
            self.symbol = self.scanner.get_symbol()
            if(self.symbol.type != phrase_type_list[i]):
                raise TypeError(
                    "Wrong type! At line " + self.symbol.line_number +
                    "and character " + self.symbol.start_char_number +
                    "expected: " + phrase_type_list[i]
                )
            elif (self.symbol.type is in (6,7,8)):
                #if symbol type is keyword, number or name
                line_output.append(self.symbol.id)
            elif (i == len-1 and self.symbol.type = phrase_type_list[len - 1]):
